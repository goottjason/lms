...
public interface CourseModifyMapper {
  ...
  @Select("select c.id, c.is_in_progress, c.name, c.number_of_learner, c.start_date, c.end_date, c.total_hours, c.total_days, c.daily_hours, c.break_time, c.lesson_start_time, c.lesson_end_time, c.lunch_start_time, c.lunch_end_time, inst.fullname as fulltimeInstructorFullname, case when sd_inst.leave_date is null then inst.id else 0 end as instructorId, admin.fullname as courseHeadFullname, case when sd_admin.leave_date is null then admin.id else 0 end as administratorId , cr.name as classroomName, cr.id as classroomId \n"
          + "from course c join staff_assignment sa_inst on c.id = sa_inst.course_id join user inst on sa_inst.user_id = inst.id and inst.type = 'INSTRUCTOR' join staff_detail sd_inst on sd_inst.user_id = inst.id join staff_assignment sa_admin on c.id = sa_admin.course_id join user admin on sa_admin.user_id = admin.id and admin.type = 'ADMINISTRATOR' join staff_detail sd_admin on sd_admin.user_id = admin.id join classroom_allocation ca on ca.course_id = c.id join classroom cr on cr.id = ca.classroom_id where c.id = #{courseId}")
