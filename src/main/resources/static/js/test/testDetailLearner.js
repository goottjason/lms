...
$(document).ready(async function () {
  ...
  try {
    ...
    $("#test-start-btn").attr("data-test-id", testId)
                        .data("test-id", testId);
    $("#test-start-btn")
    .attr("data-course-name", UrlUtils.getQueryParam("courseName"))
    .data("course-name", UrlUtils.getQueryParam("courseName"));

    $("#test-start-btn").attr("data-current-page-no", currentPageNo)
...
function makeTestDetailPageBodyForLearner(submissionStatus) {
  const testDetailBody = {
    ...
    <div class="card-body text-gray-800 pl-5 py-5">
      <p>시험은 <strong>한 번만</strong> 응시할 수 있습니다.</p>
      <p>제한 시간이 지나면 <strong>자동으로 제출</strong>됩니다.</p>
    ...
    <div class="card-footer">
      <div class="form-check">
        <input class="form-check-input test-caution-check" id="a" type="checkbox">
