...
function renderProgressSummary(summary) {
  ...
  items.forEach(({ key, selector, color, alwaysFilled }) => {
    ...
    if (alwaysFilled) {
      // 과정진행률: 항상 꽉 찬 배경
      $box
      .removeClass(`border border-${color} border-3 bg-transparent text-dark`)
    ...
    slice.forEach(card => {
      ...
      if (isHw) {
        ...
        const m = testStatusMap[card.submissionStatus]
            || { text: card.submissionStatus, class: "text-secondary" };
        statusText = m.text;
        statusClass = m.class;
      }
      ...
      $body.append(
          $("<div>").addClass("mb-3").append(
              $("<span>")
              .addClass(`badge ${badgeClass} text-white px-3 py-1 rounded-pill`)
              .text(typeText)
          )
      );
      ...
      if (isHw) {
        const evalBadge = card.score === 1
            ? $("<span>").addClass(
                "badge border-success text-success bg-transparent ms-2").text(
                "평가완료")
            : $("<span>").addClass(
                "badge border-warning text-warning bg-transparent ms-2").text(
                "평가대기");
...
function renderDebate(data) {
  ...
  data.forEach(item => {
    ...
    $titleTd.append(
        ...
        .append(
            $("<i>").addClass("fa fa-comment me-1").attr("aria-hidden", "true"),
